...
class LLaMA:
    ...
    def build(
        ...
        # This method build is returning an instance of LLaMA
        # we are correctly passing the initi params to this instance!
    ...
    def text_completion(
        ...
        # if the max_seq_len is given too big, we will take (max_gen_len + max_prompt_len) instead
        # lesser computation easy way
        # Equivalent to max_seq_len for intuition
        ...
        for i, t in enumerate(prompt_tokens):
            ...
            # later tokens are already pad_tokens!
            tokens[i, : len(t)] = torch.tensor(t, dtype=torch.long, device=device)

        eos_reached = torch.tensor(
            [False] * batch_size, device=device
        )  # initialse to False for every input

        # Masks - same shape as tokens (batch_size, total_len)
        # True if the token is a prompt token, False otherwise
        prompt_tokens_mask = tokens != pad_id

        for cur_pos in tqdm(range(1, total_len), desc="generating tokens"):
            with torch.no_grad():
                # only feeding in one token per sequence in the batch (the token at position cur_pos - 1)
            ...
            # Where we have actual prompt tokens (indicated by True in the mask), we use those tokens as-is.
            # Where we don't have prompt tokens (indicated by False in the mask), we use the predicted next_token.
            next_token = torch.where(
                condition=prompt_tokens_mask[:, cur_pos],  # (B,)
                input=tokens[:, cur_pos],  # (B,)
                other=next_token,
            )

            # tokens shape: (B, total_len)
            # update the column at position cur_pos for all batches
            tokens[:, cur_pos] = next_token

            # EOS is reached only if we found an EOS token for a padding position
            # |= is inplace OR i.e. eos_reached is updated inplace
            # Shape (B, ) - was initialised with False for each row
        ...
        # out_tokens, out_text: list of B lists
...
if __name__ == "__main__":
    ...
    assert len(out_text) == len(
        prompts
    ), "Internal error: mismatch in generated output lengths and input prompts"
